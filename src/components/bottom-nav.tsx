...
export function BottomNav({ current = "/" }: { current?: string }) {
  return (
    <nav className="sticky bottom-0 z-30 mt-auto grid grid-cols-5 gap-2 rounded-t-[20px] border-t border-[var(--border-default)] bg-[var(--surface-strong-glass)] p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] shadow-[0_-8px_32px_rgba(2,6,23,0.24)] backdrop-blur-[16px] lg:hidden safe-area-bottom">
      {items.map((item) => {
          ...
          <Link
            ...
            className={cn(
              ...
              active
                ...
                : "text-[var(--text-muted)] hover:-translate-y-[1px] hover:bg-[var(--interactive-active)] hover:text-[var(--text-primary)]",
            )}
          >
