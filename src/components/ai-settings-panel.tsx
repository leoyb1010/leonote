    ...
    <GlassPanel blur="xl" contentClassName="space-y-3 sm:space-y-4" className="rounded-[var(--radius-lg)] p-3.5 sm:p-5">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="space-y-2 text-sm text-[var(--text-secondary)]">
          <span>Base URL</span>
          <input value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} className="w-full rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-base)] h-10 px-4 text-[var(--text-primary)] outline-none focus:border-[var(--border-focus)]" />
        </label>
        <label className="space-y-2 text-sm text-[var(--text-secondary)]">
          ...
          <input value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="留空则保持当前" className="w-full rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-base)] h-10 px-4 text-[var(--text-primary)] outline-none placeholder:text-[var(--text-placeholder)]" />
        </label>
        <label className="space-y-2 text-sm text-[var(--text-secondary)]">
          <span>主模型</span>
          <select value={model} onChange={(e) => setModel(e.target.value)} className="w-full rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-base)] h-10 px-4 text-[var(--text-primary)] outline-none">
        ...
        <label className="space-y-2 text-sm text-[var(--text-secondary)]">
          <span>备用模型</span>
          <select value={fallbackModel} onChange={(e) => setFallbackModel(e.target.value)} className="w-full rounded-[var(--radius-sm)] border border-[var(--border-default)] bg-[var(--surface-base)] h-10 px-4 text-[var(--text-primary)] outline-none">
            <option value="deepseek-v4-pro">deepseek-v4-pro</option>
            <option value="deepseek-v4-flash">deepseek-v4-flash</option>
          </select>
        </label>
