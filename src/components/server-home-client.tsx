...
export function ServerHomeClient({ data, signedIn }: { data: HomeViewData | null; signedIn: boolean }) {
  if (!signedIn) {
    return <GlassPanel blur="lg" glow="soft" className="rounded-[var(--radius-lg)] p-5 text-sm leading-7 text-[var(--text-secondary)">当前未登录。先去 <Link href="/login" className="font-medium text-[var(--text-primary)] underline underline-offset-4">登录</Link>，再进入你的工作台。</GlassPanel>;
  }
    ...
    <div className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        ...
        <div className="grid grid-cols-3 gap-3 xl:grid-cols-1">
          <GlassPanel blur="lg" glow="soft" className="rounded-[var(--radius-lg)] p-4 text-center"><div className="text-3xl font-semibold text-[var(--text-primary)]">{data.counts.total}</div><div className="mt-1 text-xs text-[var(--text-muted)]">最近活跃</div></GlassPanel>
          <GlassPanel blur="lg" glow="soft" className="rounded-[var(--radius-lg)] p-4 text-center"><div className="text-3xl font-semibold text-[var(--text-primary)]">{data.counts.favorite}</div><div className="mt-1 text-xs text-[var(--text-muted)]">收藏</div></GlassPanel>
          <GlassPanel blur="lg" glow="soft" className="rounded-[var(--radius-lg)] p-4 text-center"><div className="text-3xl font-semibold text-[var(--text-primary)]">{data.counts.pinned}</div><div className="mt-1 text-xs text-[var(--text-muted)]">置顶</div></GlassPanel>
        </div>
      </div>
        ...
        <GlassPanel blur="lg" glow="brand" className="rounded-[var(--radius-lg)] p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(data.tags.length ? data.tags : ["暂无标签"]).map((tag, index) => (
              <Link key={tag} href={tag === "暂无标签" ? "/notes" : `/notes?tag=${encodeURIComponent(tag)}`} className={`rounded-full px-4 py-2 text-sm ${index === 0 ? "bg-[var(--primary)] text-white text-[var(--text-primary)]" : "border border-[var(--border-default)] bg-[rgba(255,255,255,0.06)] text-[var(--text-secondary)]"}`}>
                {tag}
              </Link>
            ))}
          </div>
