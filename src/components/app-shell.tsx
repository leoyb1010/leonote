          ...
          <GlassPanel blur="xl" className="noise-mask relative flex min-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-[32px] px-4 py-4 sm:px-5 md:px-6 lg:px-8">
            ...
            <header className="relative z-10 mb-6 flex flex-col gap-4 border-b border-[var(--border-default)] pb-5 md:flex-row md:items-start md:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href="/search" className={buttonClass("secondary", "lg", "rounded-full")}><Search className="h-4 w-4" /> 搜索</Link>
                <Link href="/settings" className={buttonClass("secondary", "lg", "rounded-full")}><Bell className="h-4 w-4" /> 设置</Link>
                <Link href="/notes/new" className={buttonClass("primary", "lg", "rounded-full")}><Plus className="h-4 w-4" /> 新建</Link>
              </div>
            </header>
