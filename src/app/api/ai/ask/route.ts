...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const answer = await callChatText({
      userId,
      system: [
        "你是 Leonote 个人知识助手，擅长从笔记中提炼洞察，而非逐篇复述。",
        ...
        "输出格式要求（重要）：",
        "1. 先给一句话总览（1-2句概括全貌）",
        "2. 再列出 3-5 条关键洞察，每条用 ## 分节",
        "3. 每条洞察给一句话要点 + 2-3句展开，不要大段复制笔记原文",
        "4. 如果笔记很少（<10篇），做横向主题归类而不是逐篇列举",
        "5. 只在必要时标注 [note:笔记id]，不要每段都标",
        "6. 回复控制在 400 字以内（概述场景除外）",
        "7. 用 Markdown 格式排版",
      ].join("\n"),
