  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(320px,var(--chat-width))]" style={{ ["--chat-width" as string]: `${width}px` }}>
        ...
        <AnimatePresence initial={false}>
          {summaryCard ? (
            <motion.div layout initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>
              <GlassPanel blur="lg" glow="brand" className="rounded-[var(--radius-lg)] p-4">
                <div className="flex items-center gap-2 text-[11px] font-semibold text-[var(--ai-accent)]"><Wand2 className="h-3.5 w-3.5" /> AI Summary</div>
                <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-[var(--text-secondary)]">{summaryCard}</p>
              </GlassPanel>
            </motion.div>
          ) : null}
        </AnimatePresence>
        ...
        <div className="columns-1 gap-4 md:columns-2">
          {memories.map((memory) => (
            <motion.div key={memory.id} layout className="relative mb-4 break-inside-avoid">
              <div className={cn("absolute right-full top-1/2 hidden h-px w-10 -translate-y-1/2 bg-gradient-to-r from-[var(--ai-accent)]/60 to-transparent xl:block", highlightedIds.includes(memory.id) && "animate-[pulse_900ms_ease-in-out_2]")} />
              <GlassPanel
                blur="lg"
                glow={highlightedIds.includes(memory.id) ? "brand" : "soft"}
                className={cn("rounded-[var(--radius-md)] p-4 transition duration-300", highlightedIds.includes(memory.id) && "ring-1 ring-[var(--ai-accent)]/40")}
              >
                <div className="flex items-center justify-between gap-2 text-[11px] font-semibold text-[var(--text-muted)">
                  ...
                  <BrainCircuit className="h-3.5 w-3.5 text-[var(--ai-accent)]" />
                </div>
                <h4 className="mt-3 text-sm font-medium text-white">{memory.label}</h4>
                {memory.content ? <p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">{memory.content}</p> : null}
              </GlassPanel>
            </motion.div>
          ))}
        </div>
      ...
      <motion.aside drag="x" dragConstraints={{ left: -220, right: 180 }} dragElastic={0.06} onDragEnd={(_, info) => setWidth((value) => Math.max(340, Math.min(720, value + info.offset.x)))} className="relative">
        <GlassPanel blur="xl" glow="brand" hoverGlow className="relative overflow-hidden rounded-[var(--radius-lg)] p-5">
          <AISpark density={10} subdued className="opacity-80" />
          <div className="relative z-10 flex items-start justify-between gap-3">
            <div>
              <div className="text-[11px] font-semibold text-[var(--text-muted)">AI Drawer</div>
              <h3 className="mt-2 text-lg font-semibold text-white">围绕当前笔记思考</h3>
            </div>
            <div className="rounded-full border border-[var(--border-default)] bg-[rgba(255,255,255,0.06)] px-3 py-1 text-xs text-[var(--text-muted)]">可拖拽宽度</div>
          </div>

          <div className="mt-4 flex flex-wrap gap-3">
            <button type="button" onClick={() => void runSummary()} disabled={!!loading} className="rounded-full bg-white px-4 py-2 text-sm font-medium text-[var(--text-primary)] transition hover:brightness-110 disabled:opacity-60">{loading === "summary" ? "生成中" : "总结这篇笔记"}</button>
            <button type="button" onClick={() => void extractMemory()} disabled={!!loading} className="rounded-full border border-[var(--border-default)] bg-[rgba(255,255,255,0.06)] px-4 py-2 text-sm text-[var(--text-muted)] transition hover:bg-[rgba(255,255,255,0.10)] disabled:opacity-60">{loading === "memory" ? "提取中" : "提取长期记忆"}</button>
          </div>

          <motion.div variants={staggerContainer} initial="initial" animate="animate" className="mt-5 space-y-3">
            {messages.length === 0 ? (
              <div className="rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[rgba(255,255,255,0.05)] px-4 py-4 text-sm leading-7 text-[var(--text-muted)]">可以直接问：这篇笔记的核心决策是什么？它和我过往哪些长期记忆冲突或一致？</div>
            ) : (
              messages.map((chat) => (
                <motion.div key={chat.id} variants={staggerItem}>
                  <motion.div
                    ...
                    whileTap="whileTap"
                    className={cn(
                      "rounded-[var(--radius-md)] border p-4 text-sm leading-7",
                      chat.role === "assistant"
                        ...
                        : "border-[var(--border-default)] bg-[rgba(255,255,255,0.06)] text-[var(--text-secondary)]",
                    )}
                  >
                    <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold text-[var(--text-muted)]">
                      {chat.role === "assistant" ? <Sparkles className="h-3.5 w-3.5 text-[var(--ai-accent)]" /> : null}
                    ...
                    {chat.memoryRefs?.length ? (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {chat.memoryRefs.map((ref) => (
                          <span key={ref.id} className="rounded-full border border-[var(--ai-accent)]/16 bg-[var(--ai-soft)] px-2.5 py-1 text-xs text-[var(--ai-accent)]">
          ...
          <div className="mt-5 overflow-hidden rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[rgba(8,11,18,0.76)] p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
            <div className="rounded-[var(--radius-sm)] bg-[rgba(8,11,18,0.88)] px-3 py-3 bg-transparent focus-within:bg-transparent transition duration-500">
              <textarea value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="围绕当前笔记继续追问…" className="min-h-[96px] w-full resize-none bg-transparent text-sm leading-7 text-[var(--text-secondary)] outline-none placeholder:text-[var(--text-placeholder)" />
              <div className="mt-3 flex items-center justify-between gap-3">
                <div className="text-xs text-[var(--text-muted)]">输入框聚焦时会出现柔和光流，避免抢占阅读。</div>
                <button type="button" onClick={() => void ask()} disabled={!!loading} className="inline-flex items-center gap-2 rounded-full bg-[var(--primary)] px-4 py-2 text-sm text-white shadow-[0_14px_32px_rgba(79,70,229,0.28)] transition hover:brightness-110 disabled:opacity-60">
          ...
          <div className="mt-4 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[rgba(255,255,255,0.05)] px-4 py-3 text-sm whitespace-pre-wrap text-[var(--text-muted)]">{message}</div>
        </GlassPanel>
      </motion.aside>
    </div>
  );
